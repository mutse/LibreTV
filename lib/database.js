...
export async function initDatabase() {
  if (process.env.NODE_ENV === 'production') {
    ...
    if (process.env.DB) {
      ...
      throw new Error('DB binding not found in production environment');
    }
  } else {
...
export async function initTables() {
  if (process.env.NODE_ENV === 'production') {
    console.log('Skipping table initialization in production (Cloudflare D1). Schema should be managed via Wrangler migrations.');
    return;
  }
